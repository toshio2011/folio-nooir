...
#include "GfxLineClip.h"
#include "SpaceAdvance.h"
// Font IDs are generated by the application font build script and live in
// src/.  This library source is compiled with its own include directory, so
// use the explicit relative path instead of relying on PlatformIO to expose
...
// Tokens without RTL script codepoints are skipped with a cheap preflight, so
...
void appendShapedRtlTokens(const char* text, std::string& shapedOut) {
  ...
  while (*p) {
    ...
    if (!utf8ContainsRtlScript(token.c_str())) continue;
...
int GfxRenderer::resolveTextFontId(const int fontId, const char* text, const EpdFontFamily::Style style) const {
  ...
  const auto fbIt = fallbackFontMap_.find(scaledFontId);
  if (fbIt == fallbackFontMap_.end()) {
    return scaledFontId;  // no fallback registered for this font
  }
  const int fallbackFontId = fbIt->second;
  const auto fontIt = fontMap.find(scaledFontId);
  const auto fallbackIt = fontMap.find(fallbackFontId);
  if (fontIt == fontMap.end() || fallbackIt == fontMap.end()) {
    return scaledFontId;  // unknown primary or fallback not loaded — let the caller handle it
  }
...
int GfxRenderer::resolveArabicFallbackFontId(const int fontId, const uint32_t cp,
                                             const EpdFontFamily::Style style) const {
  const auto primaryIt = fontMap.find(fontId);
  if (primaryIt == fontMap.end() || !utf8IsArabicCodepoint(cp) || primaryIt->second.hasCodepoint(cp, style)) {
    return fontId;
  }

  int fallbackFontId = 0;
  const auto explicitIt = arabicFallbackFontMap_.find(fontId);
  if (explicitIt != arabicFallbackFontMap_.end()) {
    fallbackFontId = explicitIt->second;
  } else {
    const auto sizeIt = fontPointSizes_.find(fontId);
    if (sizeIt != fontPointSizes_.end()) {
      const auto fallbackIt = arabicFallbackByPointSize_.find(sizeIt->second);
      if (fallbackIt != arabicFallbackByPointSize_.end()) fallbackFontId = fallbackIt->second;
    }
  }

  const auto fallbackIt = fontMap.find(fallbackFontId);
  if (fallbackIt == fontMap.end() || !fallbackIt->second.hasCodepoint(cp, style)) return fontId;
  return fallbackFontId;
}
...
int GfxRenderer::getRenderedTextAdvanceX(const int fontId, const char* renderedText,
                                         const EpdFontFamily::Style style) const {
  ...
  while ((cp = utf8NextCodepoint(reinterpret_cast<const uint8_t**>(&cursor)))) {
    ...
    int32_t advanceFP = 0;
    const auto sdIt = sdCardFonts_.find(currentFontId);
...
uint8_t GfxRenderer::getFontPointSize(const int fontId) const {
  const int scaledFontId = scaleUiFontId(fontId);
  if (const auto it = fontPointSizes_.find(scaledFontId); it != fontPointSizes_.end()) {
    return it->second;
  }
  ...
  for (const auto& family : kReaderFontIds) {
    for (size_t i = 0; i < std::size(kReaderPointSizes); ++i) {
      if (scaledFontId == family[i]) return kReaderPointSizes[i];
    }
  }
  return 0;
}

int GfxRenderer::resolveFontIdForPointSize(const int fontId, const uint8_t pointSize) const {
  const int scaledFontId = scaleUiFontId(fontId);
  if (pointSize == 0) return scaledFontId;
  ...
  for (const auto& family : kReaderFontIds) {
    size_t familyIndex = std::size(kReaderPointSizes);
    for (size_t i = 0; i < std::size(kReaderPointSizes); ++i) {
      if (scaledFontId == family[i]) {
        familyIndex = i;
        break;
      }
    }
    if (familyIndex == std::size(kReaderPointSizes)) continue;

    size_t bestIndex = familyIndex;
    uint8_t bestDistance = UINT8_MAX;
    for (size_t i = 0; i < std::size(kReaderPointSizes); ++i) {
      if (fontMap.find(family[i]) == fontMap.end()) continue;
      const uint8_t distance = kReaderPointSizes[i] > pointSize
                                   ? static_cast<uint8_t>(kReaderPointSizes[i] - pointSize)
                                   : static_cast<uint8_t>(pointSize - kReaderPointSizes[i]);
      if (distance < bestDistance || (distance == bestDistance && kReaderPointSizes[i] < kReaderPointSizes[bestIndex])) {
        bestDistance = distance;
        bestIndex = i;
      }
    }
    return fontMap.find(family[bestIndex]) != fontMap.end() ? family[bestIndex] : scaledFontId;
  }

  // SD-card families intentionally remain at the loaded size. Selecting a
  // different family or an unrelated UI font would break glyph fallback and
  // publisher/user font-family expectations.
  return scaledFontId;
}
...
static bool glyphBounds(const TextRotation rotation, const EpdFontData* fontData, const EpdGlyph* glyph,
                        const int cursorX, const int cursorY, combiningMark::Bounds* bounds) {
  if (bounds == nullptr) return false;
  bounds->x0 = bounds->y0 = bounds->x1 = bounds->y1 = 0;
  if (fontData == nullptr || glyph == nullptr || glyph->width == 0 || glyph->height == 0) return false;

  if (rotation == TextRotation::Rotated90CW) {
    bounds->x0 = cursorX + fontData->ascender - glyph->top;
    bounds->y0 = cursorY - glyph->left - (glyph->width - 1);
    bounds->x1 = bounds->x0 + glyph->height - 1;
    bounds->y1 = cursorY - glyph->left;
  } else {
    bounds->x0 = cursorX + glyph->left;
    bounds->y0 = cursorY - glyph->top;
    bounds->x1 = bounds->x0 + glyph->width - 1;
    bounds->y1 = bounds->y0 + glyph->height - 1;
  }
  return true;
}

struct ArabicMarkLayoutState {
  uint32_t baseCp = 0;
  uint8_t count = 0;
  combiningMark::Bounds bounds[combiningMark::MAX_STACKED_MARKS]{};
};

// Arabic marks are still placed by the existing font-designed mark-to-base
// heuristic.  If the next mark for that same base lands on the previous mark,
// move it only along the baseline-normal axis.  No glyph data or text is
// retained and the state lives only for this drawText() call.
static void placeArabicCombiningMark(const TextRotation rotation, const uint32_t baseCp, const uint32_t markCp,
                                     const EpdFontData* fontData, const EpdGlyph* glyph, int* cursorX, int* cursorY,
                                     ArabicMarkLayoutState* previous) {
  if (previous == nullptr || cursorX == nullptr || cursorY == nullptr || !utf8IsArabicCodepoint(markCp) ||
      baseCp == 0 || !utf8IsArabicCodepoint(baseCp)) {
    if (previous) previous->count = 0;
    return;
  }

  combiningMark::Bounds candidate;
  if (!glyphBounds(rotation, fontData, glyph, *cursorX, *cursorY, &candidate)) {
    previous->count = 0;
    return;
  }

  const int direction = glyph->top > glyph->height ? -1 : 1;
  const auto axis = rotation == TextRotation::Rotated90CW ? combiningMark::StackAxis::X
                                                           : combiningMark::StackAxis::Y;
  int selectedDelta = 0;
  for (uint8_t i = 0; i < previous->count; ++i) {
    const int delta = combiningMark::stackOffsetForOverlap(candidate, previous->bounds[i], axis, direction);
    if ((direction < 0 && delta < selectedDelta) || (direction > 0 && delta > selectedDelta)) {
      selectedDelta = delta;
    }
  }

  if (rotation == TextRotation::Rotated90CW) {
    *cursorX += selectedDelta;
    candidate.x0 += selectedDelta;
    candidate.x1 += selectedDelta;
  } else {
    *cursorY += selectedDelta;
    candidate.y0 += selectedDelta;
    candidate.y1 += selectedDelta;
  }

  previous->baseCp = baseCp;
  if (previous->count < combiningMark::MAX_STACKED_MARKS) {
    previous->bounds[previous->count++] = candidate;
  } else {
    // Abnormal longer runs remain bounded; retain the most recently
    // positioned mark as the comparison representative.
    previous->bounds[combiningMark::MAX_STACKED_MARKS - 1] = candidate;
  }
}
...
void GfxRenderer::drawText(const int fontId, const int x, const int y, const char* text, const bool black,
  ...
  if (mixedArabicFallback) {
    ...
    ArabicMarkLayoutState previousArabicMark;
    while ((cp = utf8NextCodepoint(reinterpret_cast<const uint8_t**>(&textCursor)))) {
      if (utf8IsNonRenderingFormat(cp)) continue;
      int currentFontId = resolveArabicFallbackFontId(resolvedFontId, cp, style);
      auto currentFontIt = fontMap.find(currentFontId);
      ...
      if (utf8IsTextMark(cp)) {
        const EpdGlyph* combiningGlyph = currentFont->getGlyph(cp, style);
        if (!combiningGlyph) {
          previousArabicMark.count = 0;
          continue;
        }
        const auto anchor = combiningMark::anchorFor(cp);
        const int raiseBy =
            combiningMark::raiseAboveBase(anchor, combiningGlyph->top, combiningGlyph->height, lastBaseTop);
        int combiningX = combiningMark::anchorOver(anchor, lastBaseX, lastBaseLeft, lastBaseWidth,
                                                   combiningGlyph->left, combiningGlyph->width);
        int combiningY = yPos - raiseBy;
        placeArabicCombiningMark(TextRotation::None, lastBaseCp, cp, currentFont->getData(style), combiningGlyph,
                                 &combiningX, &combiningY, &previousArabicMark);
        renderCharImpl<TextRotation::None>(*this, renderMode, *currentFont, cp, combiningX, combiningY, black, style);
        continue;
      }
  ...
  while ((cp = utf8NextCodepoint(reinterpret_cast<const uint8_t**>(&textCursor)))) {
    ...
    if (utf8IsTextMark(cp)) {
      ...
      int combiningX = combiningMark::anchorOver(anchor, lastBaseX, lastBaseLeft, lastBaseWidth,
                                                 combiningGlyph->left, combiningGlyph->width);
      int combiningY = yPos - raiseBy;
      placeArabicCombiningMark(TextRotation::None, lastBaseCp, cp, font.getData(style), combiningGlyph,
                               &combiningX, &combiningY, &previousArabicMark);
...
const char* resolveVisualText(const char* text, std::string& visualBuffer, const BidiUtils::BidiBaseDir baseDir) {
  ...
  if (baseDir != BidiUtils::BidiBaseDir::RTL) {
    if (!utf8ContainsRtlScript(text)) return text;
  }
...
void GfxRenderer::drawLine(int x1, int y1, int x2, int y2, const bool state) const {
  ...
  if (!gfx::clipLineToLogicalScreen(x1, y1, x2, y2, getScreenWidth(), getScreenHeight())) return;
  if (x1 == x2) {
    if (y2 < y1) {
      std::swap(y1, y2);
    }
...
void GfxRenderer::drawBitmap(const Bitmap& bitmap, const int x, const int y, const int maxWidth, const int maxHeight,
  ...
  const size_t rowBytesSize = bitmap.getRowBytes();
  auto rowStorage = makeUniqueNoThrow<uint8_t[]>(static_cast<size_t>(outputRowSize) + rowBytesSize);
  auto* outputRow = rowStorage.get();
  auto* rowBytes = outputRow ? outputRow + outputRowSize : nullptr;
...
int GfxRenderer::getSpaceWidth(const int fontId, const EpdFontFamily::Style style) const {
  ...
  uint16_t sdSpaceAdvanceFP = 0;
  auto sdIt = sdCardFonts_.find(scaledFontId);
  if (sdIt != sdCardFonts_.end() && sdIt->second->hasAdvanceTable()) {
    const uint8_t resolvedStyle = resolveSdCardStyle(*sdIt->second, style);
    sdSpaceAdvanceFP = sdIt->second->getAdvance(' ', resolvedStyle);
    if (gfx::selectSpaceAdvanceFP(sdSpaceAdvanceFP, 0) != 0) {
      return fp4::toPixel(sdSpaceAdvanceFP);
    }
    // A partial SD-font advance table can miss the space entry. Fall through
    // to the registered EpdFont so its normal glyph-miss handler can recover
    // the metric on demand instead of returning a zero-width space.
  }

  const auto fontIt = fontMap.find(scaledFontId);
  if (fontIt == fontMap.end()) {
    LOG_ERR("GFX", "Font %d not found", scaledFontId);
    return 0;
  }

  const EpdGlyph* spaceGlyph = fontIt->second.getGlyph(' ', style);
  const int32_t fallbackAdvanceFP = spaceGlyph ? static_cast<int32_t>(spaceGlyph->advanceX) : 0;
  return fp4::toPixel(gfx::selectSpaceAdvanceFP(sdSpaceAdvanceFP, fallbackAdvanceFP));
}
...
int GfxRenderer::getSpaceAdvance(const int fontId, const uint32_t leftCp, const uint32_t rightCp,
  ...
  const int32_t fallbackAdvanceFP = spaceGlyph ? static_cast<int32_t>(spaceGlyph->advanceX) : 0;
  const int32_t spaceAdvanceFP = gfx::selectSpaceAdvanceFP(sdSpaceAdvanceFP, fallbackAdvanceFP);
...
int GfxRenderer::getKerning(const int fontId, const uint32_t leftCp, const uint32_t rightCp,
                            const EpdFontFamily::Style style) const {
  const auto fontIt = fontMap.find(scaleUiFontId(fontId));
  if (fontIt == fontMap.end()) return 0;
...
void GfxRenderer::drawTextRotated90CW(const int fontId, const int x, const int y, const char* text, const bool black,
  ...
  if (hasArabicFallbackCandidate(resolvedFontId, text, style)) {
    int lastBaseY = y;
    int lastBaseLeft = 0;
    int lastBaseWidth = 0;
    int lastBaseTop = 0;
    ...
    while ((cp = utf8NextCodepoint(reinterpret_cast<const uint8_t**>(&text)))) {
      ...
      if (utf8IsTextMark(cp)) {
        ...
        const int raiseBy =
            combiningMark::raiseAboveBase(anchor, combiningGlyph->top, combiningGlyph->height, lastBaseTop);
        int combiningX = x - raiseBy;
        int combiningY = combiningMark::anchorOverRotated90CW(anchor, lastBaseY, lastBaseLeft, lastBaseWidth,
                                                              combiningGlyph->left, combiningGlyph->width);
        placeArabicCombiningMark(TextRotation::Rotated90CW, lastBaseCp, cp, currentFont->getData(style),
                                 combiningGlyph, &combiningX, &combiningY, &previousArabicMark);
  ...
  while ((cp = utf8NextCodepoint(reinterpret_cast<const uint8_t**>(&text)))) {
    ...
    if (utf8IsTextMark(cp)) {
      ...
      int combiningY = combiningMark::anchorOverRotated90CW(anchor, lastBaseY, lastBaseLeft, lastBaseWidth,
                                                            combiningGlyph->left, combiningGlyph->width);
      placeArabicCombiningMark(TextRotation::Rotated90CW, lastBaseCp, cp, font.getData(style), combiningGlyph,
                               &combiningX, &combiningY, &previousArabicMark);
...
void GfxRenderer::freeBwBufferChunks() {
  for (size_t i = 0; i < bwBufferChunks.size(); ++i) {
    ...
    if (bwBufferChunk) {
      ...
      EpubDiagnostics::record("grayscale_bw_buffer_free", -1, -1, 0, chunkSize, 1, chunkSize, 1);
    }
  }
}
...
bool GfxRenderer::storeBwBuffer() {
  ...
  for (size_t i = 0; i < bwBufferChunks.size(); i++) {
    ...
    EpubDiagnostics::record("grayscale_bw_buffer_alloc", -1, -1, 0, chunkSize, 1, chunkSize,
                            bwBufferChunks[i] ? 1 : 0);
